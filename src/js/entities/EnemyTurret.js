...
// 暗い座の半径。ランプ本体(BEAM_LAMP_CORE_RADIUS=2)より一回り大きい「受け皿」で、
// ピボットの円(半径8)の上に敷かれる。
//
// 7→4 に詰めた。7 は「輪(最大半径6)を覆う」大きさとして選んだ値だが、輪を砲台の
// 外へ出した時点で基準はランプ本体に変わっており、値だけが取り残されていた。
// ピボットの面積の 77% を黒く覆っていて、明るい面は外周1pxのリングだけ＝実機で
// 「PIVOT に黒いマスクを掛けていないか」と指摘された。ピボットを明るくするほど
// 悪化する組み合わせで、ほぼ白(#F2F6FA)にしたことであらわになった。
// 4 なら覆うのは面積の25%で、座は「ランプのくぼみ」に見えつつ明るい面が残る
const BEAM_LAMP_BACK_RADIUS = 4;
