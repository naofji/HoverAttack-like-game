...
// 実機で「遅い」と言われて 4 → 5 にしたが、その後の実機で「5は速すぎる」と
// 言われ 4 に戻した。タイル16pxに対して4倍の余裕があるので、この速度でも
// 1フレームで壁を飛び越すことはない
export const REFLECT_BEAM_SPEED = 4;

// 帯は「節を積み上げ、節ごとに寿命で消える」形にしてある。固定長で切り出す
// 方式だと、節が反射の折れ点をまたいだときに角をショートカットする直線になり、
// 反射のたびに帯が角でがたついて見えた（実機で指摘された）
export const REFLECT_BEAM_SEGMENT_FRAMES = 2;  // 1節を閉じるまでのフレーム数。速度4なので1節=8px
export const REFLECT_BEAM_SEGMENT_LIFE = 16;   // 1節の寿命。64px(8節)ぶん生きる。実機で「この長さがいい」と確認済み

// 1発を扇型に分ける。一方向だけだと動いている自機には当たらず緊張感が無い、
// という実機の指摘。総量は変えていない（64px × 2本 = 従来の160px 1本ぶん）
