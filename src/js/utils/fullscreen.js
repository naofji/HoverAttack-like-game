 ...
 * 呼び出し側は省略していい。
 *
 * **呼ぶ場所に制約がある。** requestFullscreen はユーザー操作の直後
 * （transient activation が生きている間）でないとブラウザに拒否される。
 * ゲームループから呼ぶこと自体は問題ないが、キー入力やクリックを受けた
 * その回の更新でなければ通らない。
 *
 * @param {Element} [element] 全画面にする要素
 * @param {Document} [doc] fullscreenElement / requestFullscreen を持つオブジェクト
 */
export function enterFullscreen(element, doc) {
...
/**
 * Fullscreen API を入る／出るで切り替える。M キーの配線用。
 *
 * @param {Element} [element] 全画面にする要素
