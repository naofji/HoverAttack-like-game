...
/**
 * 面別ランキング画面の縦の座標（1366×768 前提。y はベースライン）。
 *
 * ローカルとグローバルの2段になったので、縦は素直には余っていない。
 * 数え方: 絵の帯が 84+150=234 で終わり、上段の見出し 264 → 列見出し 290 →
 * 5行が 320〜440。下段はその 214px 下（478 → 504 → 534〜654）。
 * 開始ヒントが 748 なので、下段の最終行との間に 94px 残る。
 *
 * tierGap は「上段の最終行(440)から下段の見出し(478)まで 38px 空ける」ところから
 * 決めた。段の切れ目にはこれくらい要る（行送りは 30px なので、それより広く見えないと
 * 段が変わったと読めない）。逆に下段の余裕は 748-654=94px あるので、詰まるとしたら
 * 上ではなく段の間。実機で見て窮屈なら、まずここを動かす。
 */
export const STAGE_SCREEN = {
    sceneTop: 84,
    sceneHeight: 150,
    /** 上段（LOCAL）の段見出しのベースライン */
    tierTop: 264,
    /** 段見出しから次の段見出しまで */
    tierGap: 214,
    /** 段見出しから列見出し（FASTEST TIME / HIGH SCORE）まで */
    headerOffset: 26,
    /** 列見出しから1行目まで、および行送り */
    rowHeight: 30,
    /** 列の中心（画面幅に対する比）と、列の内側の左右の余白 */
    columnCenters: [0.27, 0.73],
    columnHalfWidth: 150,
};
