...
### Task 1: バランス定数の変更（残機・撃破点・ドローンHP・ホーミング撃墜点）

純粋な定数変更のみ。`SCORE_HOMING_INTERCEPT` は現在 `CollisionManager.js` 内のローカル定数なので、`Constants.js` へ移動して import する（テスト可能化＋一元管理）。回帰ガードのテストを1本付ける。

**Files:**
- Modify: `src/js/utils/Constants.js`
- Modify: `src/js/systems/CollisionManager.js:30`（ローカル `SCORE_HOMING_INTERCEPT` を削除し import に置換）
- Modify: `src/js/ui/ScreenRenderer.js`（HOW TO PLAY等の残機説明があれば整合）
- Test: `tests/balance-constants.test.js`

**Interfaces:**
- Produces: `Constants.js` から `CARRIER_INITIAL_LIVES=1`, `CRUISE_MISSILE_SCORE=150`, `ENEMY_DRONE_SCORE=250`, `ENEMY_DRONE_HP=8`, `SCORE_HOMING_INTERCEPT=10`, `ATTACKER_CONFIGS.standard.score=300` / `.heavy.score=500` / `.rival.score=700` / `.artillery.score=900`。
...
test('rebalanced enemy scores', () => {
  ...
  assert.equal(SCORE_HOMING_INTERCEPT, 10);
});
...
Expected: FAIL（`SCORE_HOMING_INTERCEPT` は未export、値も未変更のため）。
...
```javascript
// Score awarded for intercepting an enemy homing missile in flight.
export const SCORE_HOMING_INTERCEPT = 10;
```

- [ ] **Step 4: CollisionManager.js を import に置換**

`src/js/systems/CollisionManager.js:30` のローカル定義:

```javascript
const SCORE_HOMING_INTERCEPT = 20;
```

を削除し、ファイル冒頭の Constants import に `SCORE_HOMING_INTERCEPT` を追加する（既存の import 行に加える）。`game.addScore(SCORE_HOMING_INTERCEPT)` の呼び出しはそのまま。

- [ ] **Step 5: HOW TO PLAY の残機表記を確認**
...
```bash
git add src/js/utils/Constants.js src/js/systems/CollisionManager.js src/js/ui/ScreenRenderer.js tests/balance-constants.test.js
git commit -m "balance: キャリア残機1化・撃破点/ドローンHP/ホーミング撃墜点を再調整"
```
...
- キャリア初期残機が1、配点（アタッカー300-900・巡航150・ドローン250/HP8・地雷50・ホーミング10・アイテム200）が反映。
