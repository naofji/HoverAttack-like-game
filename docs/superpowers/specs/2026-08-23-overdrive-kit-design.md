...
- 自機のまわりの輝き（`Player._drawOverdriveGlow`）。機体より**先に**描いて
  背後から漏れる光にする。手前に描くと機体が塗り潰されて被弾の点滅と紛らわしい。
  半径は自機の幅の 2.2倍（35px）。3.0 では狭い通路が丸ごと赤くなって地形が読めなかった。
- 効いている間は**金と赤を 0.06秒ごとに往復**（`OVERDRIVE_BLINK_MS`。200→120→60ms と実機で詰めた。120ms では「ゆっくり切り替わる」に見え、瞬く感じにならなかった）。金はアイテムの色、
  赤は過負荷の色で、入れ替わり続けることで「無理をして回っている」絵になる。
- 残り3秒（`OVERDRIVE_WARN_TICKS = 180`）から**金の成分が残り時間に比例して抜け**、
  最後は赤だけの明滅になる。効果が色として失われていくので、しきい値でいきなり
  演出が切り替わるより読みやすい。金が抜けても点滅が消えないよう、
  そのぶんを濃さの差（`dim`）へ振り替えている。
  - 演出の変遷: 当初は「通常＝赤の呼吸／切れかけ＝赤の明滅」。実機で
    「暗くなっただけ」に見えて予告として弱く、次に赤／金の往復へ。さらに
    ユーザーの案で「常時 金／赤、切れかけに金が抜ける」に落ち着いた（2026-08-23）。
- HUD バーは**常時は点滅させない**（残量を読む道具なので、ちらつくと数字が追えない）。
  色だけ機体と同じ規則で金→赤へ寄り、切れる直前だけ瞬く。
