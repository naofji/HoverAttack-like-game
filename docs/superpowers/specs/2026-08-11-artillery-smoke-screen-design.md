...
半径は同じ `u` で 16 → 34px へ単調に拡大し、色段も同じ `u` で白 → 淡い紫 → 紫灰へ進む。
...
落ちないようにするための遅延実行。64×64 のオフスクリーンへ、瘤の座標表
（`[{dx, dy, r}, ...]` を4組）から焼く。グラデーションは `puffAlphaAt` から生成する。
実行時は `drawImage` ＋ 回転 ＋ 拡大 ＋ alpha のみ。
...
`_bake()` は瘤を3つ source-over で重ねて焼くため、合成後の実効 alpha は中間半径で
`falloff` の2〜3倍濃くなる（レビュー時の実測）:
...
縛っているのもそこ）。スプライトは瘤を3つ重ねて焼くので、合成後の実効 alpha は
...
SMOKE_DRIFT_SPEED         = 0.05  // 外向きの初速。**パフ自身の成長より遅くすること**（下記）
