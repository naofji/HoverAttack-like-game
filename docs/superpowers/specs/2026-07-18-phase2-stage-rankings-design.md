...
- 走行終了時（名前確定時）に、各面結果へ入力名・国旗コードを付与してローカル＆GASへ送信。
  - 送信は **名前が確定した場合のみ**（名前が無いと記録の帰属が不明なため）。
  - **ネーミングへ進む条件（2026-07-18 改訂）**: 従来は全体ハイスコア（`isHighScore`）のみ。
    改訂後は **全体ハイスコア OR いずれかのクリア済み面がトップ5相当**（`_anyStageWouldRank`）。
    → 最終面までクリアできなくても、面別記録が残せる見込みがあればネーミングに進める。
  - 切り分け: **全体（週間）ランキングへの登録は全体ハイスコア時のみ**（`isHighScore` を名前確定時にも再判定）。
    **面別ランキングへの登録は面別条件を満たす分**（`stageResults` を送信、`addStageResult` が各面トップ5を維持）。
...
- 面別の殿堂入りアーカイブ。
- ~~全体ハイスコア未達時でも面別記録を残す仕組み。~~ → 2026-07-18 に対象化・実装済み（上記「記録する値」参照）。
