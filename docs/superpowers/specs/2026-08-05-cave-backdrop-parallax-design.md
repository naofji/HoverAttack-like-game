...
- 遠景(洞窟) = 新規のオフスクリーンcanvas。カメラの25%の速度でしかスクロールしない。
...
- 写実的な描写はしない。前景がドット絵である以上、遠景も同じ描画言語(フラット塗りの
  角ばった多角形)に収める。
...
| ファイル | 変更 |
|---|---|
| `src/js/utils/Constants.js` | `FAR_BG_PARALLAX = 0.25` を追加 |
...
視差係数 `P = FAR_BG_PARALLAX = 0.25`。
実機で 0.15 は弱すぎたため 0.25 に引き上げた (2026-08-05)。
...
| マップ | 寸法 | 遠景canvas |
|---|---|---|
| 最小 (150×75タイル, missionLevel 0) | 2400×1200 | 1368×891 |
| 最大 (300×150タイル, missionLevel 4+) | 4800×2400 | 1968×1191 |

最大マップで約 1968×1191px ≒ 8.9MB。
...
- `floor` する理由: サブピクセル座標での `drawImage` は補間が走って遅く、暗い遠景の
  輪郭が滲んでチラつく。整数化により遠景はカメラが4px動くごとに1px動く階段状になるが、
  ドット絵の遠景としてはむしろ自然。
...
シェイク量も自動的に25%になる。遠くの物ほど揺れないという挙動は物理的に正しく、
意図的にこのまま残す。
...
前景 (`Map._drawRockyBlock`) は **面取り多角形のフラット塗り** だけで描かれている。
ハードエッジのみ、陰影は5段階に量子化したベタアルファ、最小ディテールは4px
(面取りと影ノッチ)。グラデーションは一切使っていない。

遠景がこの言語から外れると、ドット絵の前景に対して遠景だけ浮く。したがって:

- **グラデーション禁止** — `createRadialGradient` を使わず、すべてフラット塗りの `fill()`
- **1〜2pxの細片禁止** — 前景の最小ディテールより細かい粒を置かない
- **階調は3段階のみ** — 前景の5段階より粗くすることで「遠い」と読ませる
- **形は前景タイルの6倍スケール** — 遠景ほど形が大きく単純に見えるのはドット絵の定石
...
| 用途 | t | 説明 |
|---|---|---|
| 空洞 | 0.90 | canvas全面の基準色 |
| 岩の陰 | 0.82 | 岩塊の本体 |
| 岩のハイライト | 0.72 | 層の上端だけに乗る |

ステージごとに背景の色調が変わる(ステージ4=暗緑、ステージ5=暗青 など)。

遠景は目立ってはならない。暗すぎれば黒一色に潰れ、明るすぎれば前景のシルエットと
競合する。両側の限界を輝度テストで固定する: 空洞は純黒でない (輝度≥3)、
岩と空洞の輝度差は 6〜30、ハイライトは前景ブロックの輝度の45%以下。

### 描画手順

すべて派生 `SeededRNG` から乱数を取るため、同一シードで同一の絵になる。

1. **空洞**: 空洞色でcanvas全面を `fillRect`。これが唯一の `fillRect`。
2. **岩の層**: 高さ260pxごとに1枚 (最大マップで4枚)。各層は:
   - 上端・下端とも、96px間隔 (前景タイルの6倍) の頂点を持つ折れ線。頂点のYは±17pxで揺らす
   - 厚み 70〜160px
   - 下端の各区間に確率45%で **鍾乳石** — 底辺44px・長さ36〜126pxの三角形を折れ線に差し込む
   - 上端にだけ厚さ14pxのハイライト帯を重ね、光が上から来ている印象を作る

層と層の間が空洞として抜けて見えることで、奥に洞窟空間が広がっている印象になる。

不透明度は使わない (`ctx.globalAlpha` も `rgba()` も不要)。すべて不透明なフラット塗り。
`globalAlpha` は状態が残る副作用があり、疑似ctxで記録されないためテストで検証できない。

生成はマップ読み込み時の1回のみ。全面塗り1回 + 多角形8枚 (最大マップ) だけなので、
前景tileCacheが数万ブロックを描くコストに比べれば無視できる。
...
- 視差が弱すぎ/強すぎないか → `FAR_BG_PARALLAX` 1つで調整可能 (0.15→0.25 に調整済み)。
- 岩の層の数・形の粗さ → `BAND_SPACING` / `SEGMENT_WIDTH` で調整可能。
- 遠景のコントラストが前景のシルエット視認性を損なっていないか → `*_DARKEN` の3値で調整可能。
