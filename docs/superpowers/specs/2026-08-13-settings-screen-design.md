...
**「途中終了」だけは警告色（`UI.warn`）で描く。** この一覧で唯一「設定を変える」行では
なく、進行を捨てる行なので、同じ色で並んでいると設定項目だと思って `Enter` を押しかねない。
表の行に `danger: true` を持たせ、`drawSettings()` がそれを見る（`dimWhen` と同じ、
行に持たせるオプション）。**選択中も警告色のまま**にするのが要点で、選ぶと通常の選択色に
変わる作りだと「`Enter` を押す直前にだけ危険の手掛かりが消える」という逆の挙動になる。
選択されていることは `▶` カーソルと太字が示すので、色を選択表示に使わなくても伝わる。
