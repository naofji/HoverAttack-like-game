...
test('沈んだ岩の下面には水面を作らない（ブロックの下に線が出ない）', () => {
    // 実機の指摘:「水面下のはずなのにブロックの下面に水面が表示される」。
    // 岩の下のセルは量子化のせいで満水に1つ届かないことがよくあり、
    // 旧ルール（同じ行の区間に岩でない天井が1つでもあれば水面）だと、
    // 同じ行のどこかに本物の液面がありさえすれば、深く沈んだ岩の下にまで
    // 液面の線が引かれていた。覆っている岩の上に水が乗っているなら沈んでいる。
    const m = fromArt(`
        ##########
        #.8......#
        #.#......#
        ...
        ##########
    `);
    // (3,5) は空気の直下の水＝本物の液面。(3,2) は同じ行の区間にいるが、
    // 真上の岩の、そのまた上に水が乗っている＝沈んでいる
    assert.equal(m.kind[3 * m.cols + 5], WATER_SURFACE, '(3,5) は空気の直下なので水面');
    assert.equal(m.kind[3 * m.cols + 2], WATER_BODY,
        '(3,2) は岩に覆われ、その岩の上に水があるので水面にしてはいけない');
    assert.equal(m.surfaceY[3 * m.cols + 2], -1, '沈んだセルは自分の液面を持たない');
});

test('水の無い横穴が同じ行に伸びていても、それは液面ではない', () => {
    // 上のケースの左右版。区間の端が乾いた横穴に伸びているだけでも
    // 「天井が岩でない」には当たってしまうので、旧ルールでは水面になっていた。
    // 空気に face しているのが**水セル**であることまで見る。
    const m = fromArt(`
        #########
        #####...#
        #88#7...#
        ...
        #########
    `);
    assert.equal(m.kind[2 * m.cols + 4], WATER_BODY,
        '(2,4) の右は乾いた横穴なので、そこを液面の根拠にしてはいけない');
});
