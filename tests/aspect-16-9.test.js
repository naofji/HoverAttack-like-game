...
//
// BASE_K には baseLaser があるのに、ここには居ない。基地レーザーは弾に
// 寿命(lifetime)を持たせて距離を打ち切る方式ではなく、BASE_LASER_RANGE
// （= CANVAS_WIDTH * 0.55）そのものを毎フレームの射程チェックに使っている。
// つまり「索敵の横半径」と「弾の届く距離」が同じ CANVAS_WIDTH 由来の式を
// 指しているので、画面幅が変わっても両者は自動的に一致したまま動く。
// ドローンのような「届かない帯」が原理的に発生しないので、対にする
// RANGE_COVERAGE の行を持たない。
