...
python3 -m http.server 8000           # 実機確認用。ブラウザはハードリロードが要る
```

**実機での見た目・音の確認は必ずユーザーが行う。** こちらはサーバーを立てて案内するところまで。
...
- リーダーボードのスプレッドシートに `StageScores` シート作成 ＋ GAS Web アプリの再デプロイ。これが済むまでオンラインの面別ランキングは動かない。手順は `docs/superpowers/specs/2026-07-15-gas-setup.md`。
