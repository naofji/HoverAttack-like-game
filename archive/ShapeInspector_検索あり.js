...
export default function App() {
    ...
    // インクリメンタルサーチ: クエリが変わったら最初のマッチ箇所へジャンプ
    const prevSearchParamRef = useRef(activeSearchParam);
    useEffect(() => {
        if (activeSearchParam !== prevSearchParamRef.current) {
            prevSearchParamRef.current = activeSearchParam;
            if (isSearchBoxVisible && activeSearchParam && searchMatches.length > 0) {
                scrollToLine(searchMatches[0]);
            }
        }
    }, [activeSearchParam, searchMatches, isSearchBoxVisible, scrollToLine]);
    ...
    // (この useEffect は searchMatches で代替されるため削除)
    ...
    useEffect(() => {
        const handleKeyDown = (e) => {
            ...
            if (isCmdOrCtrl && e.key.toLowerCase() === 'f') {
                e.preventDefault();
                // テキスト選択中なら自動で検索ボックスに入力
                const selection = window.getSelection().toString().trim();
                if (selection && !selection.includes('\n')) {
                    setActiveSearchParam(selection);
                ...
                <div className="flex flex-col border-t border-slate-800 bg-[#0d1117]" style={{ height: `${bottomHeight}px`, minHeight: '100px' }}>
                    <div className="flex items-center justify-between p-2 border-b border-slate-800 bg-slate-900 shadow-sm z-10 shrink-0">
                        <div className="flex items-center gap-2 text-purple-400">
                            ...
                            <span className="text-[10px] text-slate-500 font-normal ml-2 hidden sm:inline">(ダブルクリックで直接編集 / Ctrl+Fで検索)</span>
