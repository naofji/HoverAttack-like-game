...
const analyzeCodeWithAI = async (code) => {
    ...
    const payload = {
        ...
        systemInstruction: { parts: [{ text: "You are an expert canvas 2D graphics analyzer and refactoring tool. Your task is to identify hardcoded visual values in the provided code (e.g., hardcoded radius in ctx.arc, dimensions in fillRect, colors, offsets, etc.) and smartly refactor them into configurable parameters using the `p` object (e.g., change `ctx.arc(0, 0, 8, ...)` to `ctx.arc(0, 0, p.radius, ...)`). If the code already uses variables from `p`, preserve them. Return ONLY a valid JSON object with exactly TWO keys: 1. 'rewrittenCode': The complete refactored source code string. 2. 'parameters': An object containing the metadata for all visual parameters. For 'parameters', deeply infer 'min' and 'max' (e.g., radius min:0). For categorical variables in switch/if-else blocks, return an 'options' array. Example format: {\"rewrittenCode\": \"ctx.beginPath();\\nctx.arc(0, 0, p.radius, 0, Math.PI * 2);\\nctx.fillStyle = p.bodyColor;\\nctx.fill();\", \"parameters\": {\"radius\": {\"value\": 8, \"min\": 0, \"max\": 100}, \"bodyColor\": {\"value\": \"#ff0000\"}}}" }] },
...
export default function App() {
    ...
    const runAiAnalysis = useCallback(async (code, currentParams) => {
        ...
        try {
            ...
            if (aiResult) {
                let extractedParams = aiResult;
                let newCode = code;

                if (aiResult.rewrittenCode && aiResult.parameters) {
                    extractedParams = aiResult.parameters;
                    newCode = aiResult.rewrittenCode;
                }
                ...
                Object.keys(extractedParams).forEach(key => {
                    if (typeof extractedParams[key] === 'object' && extractedParams[key] !== null && 'value' in extractedParams[key]) {
                        if (currentParams[key] !== undefined) {
                            newParams[key] = currentParams[key];
                        } else {
                            newParams[key] = extractedParams[key].value;
                        }

                        newMeta[key] = {};
                        if (extractedParams[key].min !== undefined && extractedParams[key].max !== undefined) {
                            newMeta[key].min = extractedParams[key].min;
                            newMeta[key].max = extractedParams[key].max;
                        }
                        if (extractedParams[key].options !== undefined && Array.isArray(extractedParams[key].options)) {
                            newMeta[key].options = extractedParams[key].options;
                        }
                    } else {
                        if (currentParams[key] !== undefined) {
                            newParams[key] = currentParams[key];
                        } else {
                            newParams[key] = extractedParams[key];
                        }
                    }
                });
    ...
    const searchMatches = useMemo(() => {
        if (!activeSearchParam) return [];

        if (parsedParams[activeSearchParam] !== undefined && paramToLinesMap[activeSearchParam]) {
            return paramToLinesMap[activeSearchParam];
        }

        const lines = sourceCode.split('\n');
        const matches = [];
        try {
            const escaped = activeSearchParam.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            const regex = new RegExp(escaped, 'gi');
            lines.forEach((line, idx) => {
                if (regex.test(line)) matches.push(idx);
            });
        } catch (e) { }
        return matches;
    }, [sourceCode, activeSearchParam, paramToLinesMap, parsedParams]);

    const currentMatchIdx = searchMatches.length > 0 ? Math.min(searchMatchIndex, searchMatches.length - 1) : 0;
    ...
    const highlightText = useCallback((text, keyword) => {
        ...
        let parts = [];
        if (parsedParams[keyword] !== undefined) {
                    ...
                    <div className="bg-slate-900 border border-slate-700 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden">
                        ...
                        <div className="p-6 space-y-4">
                            <div className="grid grid-cols-[100px_1fr] gap-4 items-center">
                                ...
                                <kbd className="px-2 py-1.5 bg-slate-800 border border-slate-700 rounded text-xs font-mono text-center text-slate-300 shadow-[0_2px_0_rgba(51,65,85,1)]">Ctrl + F</kbd>
                                <span className="text-sm text-slate-300">コード内の文字列を検索します</span>
                ...
                <div className="flex flex-col border-t border-slate-800 bg-[#0d1117]" style={{ height: `${bottomHeight}px`, minHeight: '100px' }}>
                    <div className="flex items-center justify-between p-2 border-b border-slate-800 bg-slate-900 shadow-sm z-10 shrink-0">
                        <div className="flex items-center gap-2 text-purple-400">
                            ...
                            {(isSearchBoxVisible || activeSearchParam !== null) && (
                                <div className="flex items-center gap-1 ml-4 bg-slate-950 px-2 py-0.5 rounded border border-slate-700">
                                    <input
                                        ref={searchInputRef}
                                        type="text"
                                        className="bg-transparent text-[10px] text-yellow-400 font-mono outline-none w-24 placeholder-slate-600 focus:w-32 transition-all"
                                        placeholder="Search..."
                                        value={activeSearchParam || ''}
                                        onChange={(e) => {
                                        ...
                                        onBlur={() => {
                                            if (!activeSearchParam) setIsSearchBoxVisible(false);
                                        }}
                                    />
                                    <div className="flex items-center ml-1 border-l border-slate-700 pl-1 gap-0.5">
                                        <span className="text-slate-400 text-[10px] mr-1 whitespace-nowrap">
                    ...
                    <div ref={codeContainerRef} className="flex-1 overflow-y-auto custom-scrollbar p-1.5 font-mono text-xs leading-relaxed min-h-0">
                        {sourceCode.split('\n').map((line, idx) => {
                            ...
                            const isSearchMatch = activeSearchParam && searchMatches.includes(idx);
                            const isCurrentSearchMatch = isSearchMatch && searchMatches[currentMatchIdx] === idx;
